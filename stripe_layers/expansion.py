...
class PiecewiseShared(nn.Module):
    def __init__(self,
                 n: int, in_channels: int, out_channels: int, segments: int, length: int = 2.0,
        ...
        self.w = torch.nn.Parameter(data=torch.Tensor(
            out_channels, in_channels, ((n-1)*segments+1)), requires_grad=True)
    ...
    def forward(self, x):
        ...
        w = w.view(self.out_channels,
                   wid_min.shape[0], self.in_channels, wid_min.shape[-1], self._n)
...
class PiecewisePolynomialShared(PiecewiseShared):
    def __init__(self, n, in_channels, out_channels, segments, length=2.0, weight_magnitude=1.0, periodicity: float = None, device='cuda', ** kwargs):
        super().__init__(n, in_channels=in_channels, out_channels=out_channels, segments=segments,
                         length=length, weight_magnitude=weight_magnitude, poly=LagrangePolyShared, periodicity=periodicity, device=device)
